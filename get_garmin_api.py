...
def insert_yesterday():
    ...
    sleep_start_jst = GMT_to_JST(sleep_start_gmt)
    sleep_end_jst = GMT_to_JST(sleep_end_gmt)
    insert_data.append([yesterday_iso, sleep_start_jst, sleep_end_jst, sleep_hours])
...
# n_day分DBに入れる
insert_data = insert_n_day(10, yesterday, yesterday_iso)
ic(insert_data)
...
with get_connection(autocommit=True) as conn:
    with conn.cursor() as cur:
        ...
        query = f"""
            INSERT INTO
                garmin (date, sleep_start_jst, sleep_end_jst, sleep_hours)
                VALUES(%s, %s, %s, %s)
            ON DUPLICATE KEY UPDATE
                sleep_start_jst = IF(sleep_start_jst <> VALUES(sleep_start_jst), VALUES(sleep_start_jst), sleep_start_jst),
                sleep_end_jst = IF(sleep_end_jst <> VALUES(sleep_end_jst), VALUES(sleep_end_jst), sleep_end_jst),
